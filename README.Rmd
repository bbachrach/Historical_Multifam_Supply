...
Keep only Residential conversions. If there are duplicated of address (PLAN_STREET), keep only the earliest instance, if ZIP is not formatted properly set to NA
```{r eval=F}
...
For the pre-existing manually coded BBLs, remove those PLAN_IDs from the AG data. Keep that slice of the AG data in a separate dataframe that is used downstream
```{r eval=F}
...
First, if an observation is classified is new or the effective date is missing, remove those observations. This is done by splitting the dataframe in two, one of which contains all of the instances which are listed as new or do not have an effective date. Remove all BBLs in the new_or_noneffective dataframe which have a corresponding BBL in the effective conversions dataframe. Bind the two back together, arrange so that the earliest conversions come first and then remove all duplicated BBLs.
```{r eval=F}
