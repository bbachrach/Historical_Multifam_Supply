...
#   
#   AGdata.df <- AGdata.df %>%
#     mutate(PLAN_ZIP = ifelse(!PLAN_ZIP %in% zips.list[[i]]
#                              
#                              
#                              ))
#   
...
# )
# AGdata.df %>%
#   # group_by(PLAN_RESIDENTIAL) %>%
#   group_by(PLAN_CONSTR_TYPE) %>%
#   summarize(count = n())
...
# tmp.out %>% filter(!is.na(bbl)) %>% group_by(returnCode1a) %>% summarize(count = n())
# View(tmp.out %>%
#        filter(!is.na(bbl) & returnCode1a %in% "88"))
# View(tmp.out2)
...
#   )
# tmp.out %>%
#   semi_join(mancoded_secondary.df
#             ,by="PLAN_ID_UNIQUE")
# 
# colnames(mancoded_secondary.df)
