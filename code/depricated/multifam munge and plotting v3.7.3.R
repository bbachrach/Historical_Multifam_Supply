...
# AGdata.df <- read.xlsx("/Users/billbachrach/Dropbox (hodgeswardelliott)/Data Science/Bill Bachrach/Major projects/UWS condo prop/Data/Coop_Condo_AGdatadata.xlsx",
#                    sheetIndex=1)
# AGdata.df <- read.csv("/Users/billbachrach/Dropbox (hodgeswardelliott)/Data Science/Bill Bachrach/Major projects/UWS condo prop/Data/Coop_Condo_AGdata.csv",
#                       stringsAsFactors=F)
...
AGdata.df <- read.csv("/Users/billbachrach/Dropbox (hodgeswardelliott)/Data Science/Bill Bachrach/Data Sources/AG Data/Coop_Condo_AGdata.csv"
           ,stringsAsFactors=F) %>%
    filter(
      !(is.na(PLAN_DATE_EFFECTIVE)
      #   | !grepl("[[:digit:]]",PLAN_DATE_EFFECTIVE)
...
getwd()
setwd("/Users/billbachrach/Dropbox (hodgeswardelliott)/Data Science/Bill Bachrach/Major projects/UWS condo prop/Data/AG data")
# saveRDS(Conversions.df,"AG_condo_coop_conversions.rds")
Conversions.df <- readRDS("/Users/billbachrach/Dropbox (hodgeswardelliott)/Data Science/Bill Bachrach/Major projects/UWS condo prop/Data/AG data/AG_condo_coop_conversions.rds")
...
pluto.all <- readRDS("/Users/billbachrach/Dropbox (hodgeswardelliott)/Data Science/Project Data/PLUTO/pluto_all_compressed_2003_2016.rds") %>%
  arrange(desc(Year)) %>%
  filter(APPBBL != 0 & !duplicated(paste(BBL))) %>%
  mutate(BBL = paste(
    as.numeric(BoroCode)
    ,as.numeric(Block)
    ,as.numeric(Lot)
...
pluto.aug <- left_join(
  pluto %>% select(BBL,Borough,Block,Lot
                   ,Easements
                   ,OwnerType,OwnerName,BldgArea,ComArea,ResArea
                   ,OfficeArea,RetailArea,GarageArea,StrgeArea,FactryArea
                   ,OtherArea,NumBldgs,NumFloors,UnitsTotal,YearBuilt,YearAlter1,YearAlter2,APPBBL
                   ,CondoNo,XCoord,YCoord)
  ,condocoop.df
  ,by="BBL"
)
...
pluto.aug %>% 
  filter(BldgClass %in% c(rental.classes,"RM") & is.na(TCO.real)) %>%
  filter(!is.na(TCO.impute) & TCO.impute>2005) %>%
  group_by(TCO.impute) %>%
  summarize(count = n()
            ,units = sum(UnitsRes.old)
            ) %>%
  arrange(TCO.impute)

pluto.aug %>% 
  filter(BldgClass %in% c(rental.classes,"RM") & !is.na(TCO.real)) %>%
  filter(!is.na(TCO.real) & TCO.real>2005) %>%
  group_by(TCO.real) %>%
  summarize(count = n()
            ,units = sum(UnitsRes.old)
  ) %>%
...
pluto.aug %>% 
  # filter(BldgClass %in% c(rental.classes) & !is.na(TCO.real)) %>%
  filter(Type %in% "all") %>%
  filter(UnitsRes.old >3) %>%
  filter(!is.na(TCO) & TCO>2005) %>%
  group_by(TCO.real) %>%
  summarize(count = n()
            ,units = sum(UnitsRes.old)
  ) %>%
...
summary(as.factor(pluto.aug %>% filter(!is.na(TCO)) %>% pull(TCO)))
summary(
  as.factor(pluto.aug[!is.na(pluto.aug[,"TCO"]),"TCO"])
)
