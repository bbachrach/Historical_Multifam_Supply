...
tmp.text <- try(suppressWarnings(pdf_text(tmp.vec[4])))

unit_standard.fun <- function(tmp.text){
  
  unit.location <- str_locate(tmp.text,"UNITS: ")
  
  if(sum(apply(unit.location,1,function(x){sum(!is.na(x))}))>0){
    unit.line <- which(
      apply(unit.location,1,function(x){sum(is.na(x))}) == 0 
    )
    
    unit.start <- unit.location[unit.line,2]
    unit.snippet <- str_sub(tmp.text[unit.line],start=unit.start,end=unit.start+40)
    
    unit.stop <- str_locate(unit.snippet," residential")[1,1]
    ...
    if(is.na(unit.stop)){
      if(grepl("non-residential",unit.snippet,ignore.case=T)){
        unit.dig <- 0}
    } else {
      unit.dig <- trimws(str_sub(unit.snippet,start=1,end=unit.stop))
    }
  } else {
    unit.dig <- NA
  }
...
# Number of Units
unit_13_14.fun <- function(tmp.text){
  unit.location <- str_locate(tmp.text,"Number of Units ")  
  ...
  if(sum(apply(unit.location,1,function(x){sum(!is.na(x))}))>0){
    ...
    unit.start <- unit.location[unit.line,2]
    unit.snippet <- str_sub(tmp.text[unit.line],start=unit.start,end=unit.start+70)
    ...
    if(is.na(unit.stop)){
      ...
      if(grepl("residential",unit.snippet,ignore.case=T)){
        unit.dig <- trimws(str_sub(unit.snippet,start=1,end=unit.stop))
      } else {
        unit.dig <- NA
      }
...
units.fun <- function(doc_year,tmp.text){
  if(doc_year %in% c(2013,2014)){
    out <- unit_13_14.fun(tmp.text)
  } else {
    out <- unit_standard.fun(tmp.text)
  }
  return(out)
}
